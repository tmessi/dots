...
# Similar to python's virtualenv but for working with composer
function phpve() {
    local ve_root=$(git rev-parse --show-toplevel 2> /dev/null)
    local vendor_dir="$(dirname $(find ${ve_root} -wholename '*/vendor/autoload.php'))"
    local vendor_bin="${vendor_dir}/bin"
    local phpenv_bin="${ve_root}/.phpenv/bin"

    mkdir -p ${phpenv_bin}
    cat > ${phpenv_bin}/activate <<EOF
...
PATH="${phpenv_bin}:${vendor_bin}:\$PATH"
export PATH
